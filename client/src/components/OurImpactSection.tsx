...
const AnimatedCounter = ({ target, duration = 2, suffix = '' }: CounterProps) => {
  ...
  return (
    <span ref={ref} className="text-[#FFD95A] font-bold text-3xl md:text-4xl">
        ...
        >
          {impactData.map((item, index) => (
            <motion.div
              ...
              className="bg-[#0F172A] rounded-lg border border-[#1E3A8A] shadow-inner p-6 text-center hover:shadow-lg hover:shadow-[#1E3A8A]/20 transition-all duration-300 group"
            >
              <div className="mb-4">
                <AnimatedCounter 
                  target={item.number} 
              ...
              <h3 className="text-white font-semibold text-lg mb-2 group-hover:text-[#FFD95A] transition-colors duration-300">
                {item.title}
              </h3>
